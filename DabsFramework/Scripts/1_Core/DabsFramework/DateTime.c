...
{		
	...
	*/
	...
	string ToString(string format)
	{
		...
		if (format.Replace("HH", hour.ToStringLen(2)) != 2) {
			format.Replace("H", hour.ToString());
		}
		
		if (format.Replace("mm", minute.ToStringLen(2)) != 2) {
			format.Replace("m", minute.ToString());
		}
		
		if (format.Replace("MMMM", MONTH_NAME_LONG[month - 1]) != 4) {
			if (format.Replace("MMM", MONTH_NAME_SHORT[month - 1]) != 3) {
				if (format.Replace("MM", month.ToStringLen(2)) != 2) {
					format.Replace("M", month.ToString());
				}
			}
		}
		
		if (format.Replace("ss", second.ToStringLen(2)) != 2) {
			format.Replace("s", second.ToString());
		}
		
		if (format.Replace("tt", MERIDIEM_LONG[hour / 12]) != 2) {
			format.Replace("t", MERIDIEM_SHORT[hour / 12]);
		}
		
		if (format.Replace("yyyy", year.ToStringLen(4)) != 4) {
