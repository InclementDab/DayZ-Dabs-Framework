...
{
	void RegisterProfileSettingColor(string category_name, string variable_name, string display_name, bool require_restart = false)
	{
		...
		foreach (ProfileSettingsCategory category: categories) {
			if (category && category.Header == category_name) {
				category.Settings.Insert(new ProfileSettingColor(variable_name, display_name, require_restart));
				return; // we found it
			}
		}
		...
		new_category.Settings.Insert(new ProfileSettingColor(variable_name, display_name, require_restart));
